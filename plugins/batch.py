...
async def clear_all_active_batches():
    global ACTIVE_USERS
    ACTIVE_USERS = {}
    await save_active_users_to_file()
...
@X.on_message(filters.command('reset'))
async def reset_cmd(c, m):
    uid = m.from_user.id
    
    if uid not in OWNER_ID:
        await m.reply_text('⚠️ This command is restricted to the bot owner.')
        return
    
    global Z
    active_count = len(ACTIVE_USERS)
    
    await clear_all_active_batches()
    Z.clear()
    
    missing_creds = []
    if not API_ID or API_ID == 0:
        missing_creds.append("API_ID")
    if not API_HASH:
        missing_creds.append("API_HASH")
    if not BOT_TOKEN:
        missing_creds.append("BOT_TOKEN")
    if not MONGO_DB:
        missing_creds.append("MONGO_DB")
    
    response = f"✅ **System Reset Complete!**\n\n"
    response += f"🗑️ Cleared {active_count} active batch operation(s)\n"
    response += f"🔄 Reset all user states\n"
    response += f"✨ System is ready for next batch\n\n"
    
    if missing_creds:
        response += f"⚠️ **Missing Credentials:**\n"
        response += f"Please configure the following in config.py or environment variables:\n"
        for cred in missing_creds:
            response += f"• {cred}\n"
    else:
        response += f"✅ All required credentials are configured"
    
    await m.reply_text(response)

@X.on_message(filters.text & filters.private & ~login_in_progress & ~filters.command([
    'start', 'batch', 'cancel', 'login', 'logout', 'stop', 'reset', 'set', 
